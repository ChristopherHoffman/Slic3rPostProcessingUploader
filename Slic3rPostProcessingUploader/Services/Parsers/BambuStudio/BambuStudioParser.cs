...
{
    internal class BambuStudioParser : IGcodeParser
    {
        private readonly string noteTemplate;

        public BambuStudioParser(string noteTemplate)
        {
            if (string.IsNullOrEmpty(noteTemplate))
            {
                var defaultTemplate = new BambuStudioDefaultNoteTemplate();
        ...
        private string RenderNoteTemplate(string gcode)
        {
            ...
            if (matches != null)
            {
                foreach (Match match in matches)
                {
                    ...
                    // Convert into BambuStudio-slicers format
        ...
        private string GetSlicerVersion(string gcode)
        {
            ...
            var printTimeStringMatch = Regex.Match(gcode, @"; BambuStudio (.+)\s", RegexOptions.IgnoreCase | RegexOptions.Multiline);
        ...
        private List<PrintFilamentSummaryDto> GetFilamentUsage(string gcode)
        {
            ...
            string filamentUsed = ParseSettingAsString(gcode, "; total filament length \\[mm\\]");
        ...
        public double? EstimateFilamentUsageInMg(string gcode)
        {
            ...
            var filamentUsedInGrams = ParseSettingAsNumber(gcode, "; total filament weight \\[g\\]");
            ...
            var filamentUsageLengthInMM = double.Parse(ParseSettingAsString(gcode, "; total filament length \\[mm\\]"));
